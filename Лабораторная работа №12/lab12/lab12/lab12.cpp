...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
    ...
    FILE* f;
    fopen_s(&f,"C:\\TEMP\\struct.bin", "rb");
    fread(&n, sizeof(int), 1, f);
    arr = (figure*)malloc(sizeof(figure) * n);
    fread(arr, sizeof(figure), n, f);
    fclose(f);
    ...
    case WM_PAINT:
        {
            ...
            for (int i = 0; i < n; i++)
            {
                HPEN Pen = CreatePen(PS_SOLID, arr[i].W, RGB(0, 0, 0));
                HBRUSH color = CreateSolidBrush(RGB(arr[i].r, arr[i].g, arr[i].b));
                SelectObject(hdc, Pen);
                SelectObject(hdc, color);
                Ellipse(hdc, arr[i].x - arr[i].R, arr[i].y - arr[i].R, arr[i].x + arr[i].R, arr[i].y + arr[i].R);
                DeleteObject(Pen);
                DeleteObject(color);
            }
